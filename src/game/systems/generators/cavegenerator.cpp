...
void CaveGenerator::randomFillMap(GameMap      &map,
...
uint8_t CaveGenerator::checkNeighbors(GameMap &map, uint32_t x, uint32_t y)
{
    ...
    for (auto column = x - 1; column <= x + 1; column++)
    {
        for (auto cell = y - 1; cell <= y + 1; cell++)
        {
            if (!(column == x && cell == y))
            {
                if ((map[column][cell].type & TileType::WALL) != TileType::NONE)
                {
                    wall_counter++;
                }
            }
...
void CaveGenerator::fourFive(GameMap &map, uint32_t size_x, uint32_t size_y)
{
...
void CaveGenerator::rectify(GameMap &map)
{
    for (auto &x : map)
    {
        for (auto &y : x)
...
uint32_t CaveGenerator::countWalls(GameMap &map)
{
    uint32_t walls = 0;
    for (auto &x : map)
    {
...
void CaveGenerator::floodFill(GameMap &map, uint32_t x, uint32_t y)
{
...
GameMap CaveGenerator::generate(std::mt19937 &twister_engine,
                                uint32_t      size_x,
                                uint32_t      size_y)
{
