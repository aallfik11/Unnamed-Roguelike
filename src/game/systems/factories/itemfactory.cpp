...
ItemBuilder &ItemBuilder::itemComponent(const ItemType type,
                                        const uint16_t stack,
                                        const uint16_t max_stack,
                                        const Rarity   rarity,
                                        const bool     equipped)
{
    components.insert(
...
ItemBuilder &ItemBuilder::name(const std::string &name)
{
    components.insert(new Name(name));
    return *this;
}

ItemBuilder &ItemBuilder::description(const std::string &description)
{
    components.insert(new Description(description));
    return *this;
}

ItemBuilder &ItemBuilder::sprite(const TileAppearance appearance)
{
    components.insert(new TileComponent(appearance));
    return *this;
}

ItemBuilder &ItemBuilder::damage(const uint16_t damage)
{
    components.insert(new WeaponComponent(damage));
    return *this;
}

ItemBuilder &ItemBuilder::crit(const uint8_t crit_chance,
...
ItemBuilder &ItemBuilder::armor(const uint8_t armor_class)
{
    components.insert(new ArmorComponent(armor_class));
    return *this;
}

ItemBuilder &ItemBuilder::buffs(std::initializer_list<EffectComponent *> buffs)
{
    components.insert(new BuffComponent(buffs));
    return *this;
}

ItemBuilder &ItemBuilder::hungerValue(const uint8_t saturation)
{
    components.insert(new HungerComponent(saturation));
    return *this;
}

std::list<Entity *> ItemFactory::generateWeapons(int amount)
{
    ...
    for (int i = 0; i < amount; i++)
    {
        ...
        if (weapon_rarity >= Rarity::RARE)
        {
            ...
            if (roll <= 2)
            {
                ...
                weapon_buffs->buffs[(effect &
                                     ~(Effect::APPLIED | Effect::APPLY_ONCE))] =
...
std::list<Entity *> ItemFactory::generateArmors(int amount)
{
    ...
    for (int i = 0; i < amount; i++)
    {
        ...
        armor_distro = std::uniform_int_distribution<uint8_t>(ac_min, ac_max);
...
std::list<Entity *> ItemFactory::generateRings(int amount)
{
    ...
    for (int i = 0; i < amount; i++)
    {
        ...
        rings.emplace_back(new Entity(
            ...
            {new ItemComponent(ItemType::RING, 1, 1, ring_rarity),
             new Name(name),
             new Description(description),
             new BuffComponent({new EffectComponent(effect, effect_strength)}),
             new TileComponent(TileAppearance::RING)}));
    }
    return rings;
}
std::list<Entity *> ItemFactory::generatePotions(int amount)
{
...
std::list<Entity *> ItemFactory::generateFood(int amount)
{
...
ItemFactory::ItemFactory(int &depth) : current_depth_{depth}
{
...
inline void ItemFactory::placeItem(observer_ptr<Entity> item)
{

    auto x = x_pos_distro_(mt_engine_);
    auto y = y_pos_distro_(mt_engine_);
    while (((*map_)[x][y].type & (TileType::HAS_CREATURE | TileType::HAS_ITEM |
                                  TileType::HAS_STAIRS | TileType::WALL)) !=
           TileType::NONE)
    {
...
std::list<Entity *> ItemFactory::generateItems() noexcept(false)
{
    if (map_ == nullptr)
    ...
    if (current_depth_ == 25)
    {
        ...
        auto        yendor =
            builder.name("Amulet of Yendor")
                .description("If you're reading this in-game, something isn't "
                             "working :)")
...
Entity *ItemFactory::generateWeapon(int rarity_modifier)
{
...
Entity *ItemFactory::generateArmor(int rarity_modifier)
{
...
Entity *ItemFactory::generateRing(int rarity_modifier)
{
...
Entity *ItemFactory::generatePotion(int rarity_modifier)
{
    int depth_backup  = current_depth_;
    current_depth_   += rarity_modifier;
...
Entity *ItemFactory::generateFoodRation(int rarity_modifier)
{
    int depth_backup  = current_depth_;
    current_depth_   += rarity_modifier;
...
std::list<Entity *> ItemFactory::debugGenerateItems(int amount) noexcept(false)
{
    if (map_ == nullptr)
        throw std::runtime_error("Item Factory: ERROR -> Map unassigned");
    std::list<Entity *> generatedItems;
...
void ItemFactory::assignMap(observer_ptr<GameMap> map) { this->map_ = map; }
