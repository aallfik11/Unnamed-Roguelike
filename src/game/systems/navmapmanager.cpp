...
void NavMapManager::resetNavMap(NavMap &nav_map)
{
...
void NavMapManager::bfsNavMap(uint16_t target_x,
                              uint16_t target_y,
                              uint16_t priority,
                              double   multiplier,
                              NavMap  &nav_map)
...
void NavMapManager::fillRandomTargets()
{
...
NavMapManager::NavMapManager()
{
    mt_engine_ = std::mt19937(rd_());
    distro_x_  = std::uniform_int_distribution<uint16_t>(1, G_MAP_WIDTH - 2);
    distro_y_  = std::uniform_int_distribution<uint16_t>(1, G_MAP_HEIGHT - 2);
    map_       = nullptr;
...
NavMapManager::NavMapManager(observer_ptr<const GameMap>      map,
...
void NavMapManager::assignMap(observer_ptr<const GameMap> map) { map_ = map; }
void NavMapManager::assignPlayer(observer_ptr<const Entity> player)
{
    player_coordinates_ = player->getConstComponent<Coordinates>();
}
void NavMapManager::initializeRandomTargets() noexcept(false)
{
    if (map_ == nullptr)
...
void NavMapManager::calculatePlayerNavMap() noexcept(false)
{
    if (player_coordinates_ == nullptr)
        throw std::runtime_error("NavMapManager: ERROR -> Player unassigned");
...
void NavMapManager::switchToPlayerNavMap(NavMap &nav_map) const noexcept(false)
{
    if (player_coordinates_ == nullptr)
        throw std::runtime_error("NavMapManager: ERROR -> Player unassigned");
...
void NavMapManager::calculateNavMap(NavMap     &nav_map,
                                    TargetTuple targets) noexcept(false)
{
...
void NavMapManager::assignRandomTarget(NavMap &nav_map) noexcept(false)
{
    if (map_ == nullptr)
        throw std::runtime_error("NavMapManager: ERROR -> Map unassigned");
    ...
    calculateNavMap(nav_map,
                    {
                        {random_target.first, random_target.second, 0, 1.0}
    });
}

bool NavMapManager::compareNavtuple(
...
NavMapManager::nextBestCoordinates(observer_ptr<Entity> const entity,
    ...
    noexcept(false)
{
    if (map_ == nullptr)
        throw std::runtime_error("NavMapManager: ERROR -> Map unassigned");
    if (entity == nullptr)
        throw std::runtime_error("NavMapManager: ERROR -> entity is nullptr");
...
void NavMapManager::updateData() { calculatePlayerNavMap(); }
void NavMapManager::readSystemMessages() {}
void NavMapManager::clearSystemMessages()
{
    (*System::system_messages_)[SystemType::NAVMAP_MANAGER].clear();
}
void NavMapManager::resetSystem()
{
    clearSystemMessages();
