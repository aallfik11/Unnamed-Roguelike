...
inline uint8_t AttackSystem::truncateArmorClass(uint8_t ac)
{
    return (ac > 100) ? 100 : ac;
}

bool AttackSystem::attack(const observer_ptr<Entity> attacker,
                          const observer_ptr<Entity> defender)
{
    ...
    const std::string miss_message = attacker->getComponent<Name>()->name +
                                     " missed " +
                                     defender->getComponent<Name>()->name;

    const std::string hit_message = attacker->getComponent<Name>()->name +
                                    " hit " +
                                    defender->getComponent<Name>()->name;

    const std::string crit_message = attacker->getComponent<Name>()->name +
                                     " critically strikes " +
                                     defender->getComponent<Name>()->name;
    ...
    if (defender_effects->buffs.contains(Effect::IRONSKIN | Effect::PERMANENT))
    {
        ac_modifier +=
...
void AttackSystem::readSystemMessages()
{
    for (auto &message : (*system_messages_)[SystemType::ATTACK])
    {
        auto message_iterator = message.begin();
        auto attacker = std::any_cast<observer_ptr<Entity>>(*message_iterator);
        auto defender =
...
void AttackSystem::clearSystemMessages()
{
