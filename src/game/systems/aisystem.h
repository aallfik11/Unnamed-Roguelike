...
class AISystem
{
  ...
  using StateMap = std::unordered_map<AIState, std::function<Action(EntityPtr, EntityPtr)>>;
  ...
  // todo: make the functions take a boolean to ignore checks (to make calling them from other functions faster);
  ...
  Action approachTarget(EntityPtr &caller, EntityPtr &target)
  {
    ...
    */
    ...
    // if player within range, attack
    auto abs_distance_x = std::abs(caller_coordinates->x - target_coordinates->x);
    auto abs_distance_y = std::abs(caller_coordinates->y - target_coordinates->y);
  ...
  Action runAway(EntityPtr &caller, EntityPtr &target)
  {
    /*if there's a line of sight to the player, run away, else rest
      if no way of running exists (backed into a corner), if not cowardly enter berserk mode
    */
    ...
    auto [x, y] = navigation_manager_.nextBestCoordinates(caller, NavMapManager::Destination::AWAY_FROM);
    auto caller_coordinates = caller->getComponent<Coordinates>();
    if (caller_coordinates->x == x && caller_coordinates->y == y)
    {
      auto caller_brain = caller->getComponent<AIComponent>();
  ...
  Action rest(EntityPtr &caller, EntityPtr &target)
  {
    ...
    */
    ...
    double current_hp_percentage = (static_cast<double>(caller_health->current_health_points) /
    ...
    caller_health->current_health_points += 1; // might want to add a regen_amount component (or a field to Health component) later
  }
  Action attack(EntityPtr &caller, EntityPtr &target)
  {
    ...
    health_system_.updateHealth(target, /*WEAPON OR DAMAGE COMPONENT GOES HERE, THIS IS A PLACEHOLDER*/ 420, HealthAction::DEDUCE);
  }
